...
const Showcase = () => {
    ...
    useGSAP(() => {
        if(!isTablet) {
            // set initial visual state: video visible, logo hidden and larger
            gsap.set('.mask img', { opacity: 0, scale: 1.5 });
            ...
            // show content first, then reveal the mask logo later in the scroll timeline
            timeline
                .to('.content', { opacity: 1, y: 0, ease: 'power1.in' }, 0.2)
                .to('.mask img', { opacity: 1, scale: 1.1, ease: 'power1.out' }, 0.6);
        }
    }, [isTablet])
